...
public class GenericItemProvider implements ItemProvider, ModelRepositoryChangeListener {
	...
	public GenericItemProvider(ModelRepository modelRepository, Collection<BindingConfigReader> readers,
			Collection<ItemFactory> factorys) {
		this.modelRepository = modelRepository;
		this.modelRepository.addModelRepositoryChangeListener(this);

		for (BindingConfigReader reader : readers)
			addBindingConfigReader(reader);

		for (ItemFactory factory : factorys)
			addItemFactory(factory);
	}
	...
	private GroupItem applyGroupFunction(GenericItem baseItem, ModelGroupItem modelGroupItem,
		...
		for (String arg : modelGroupItem.getFunction().getArgs()) {
			State state = TypeParser.parseState(baseItem.getAcceptedDataTypes(), arg);
			if (state == null) {
				logger.warn("State '" + arg + "' is not valid for group item '" + modelGroupItem.getName()
						+ "' with base type '" + modelGroupItem.getType() + "'");
				args.clear();
				break;
			} else {
				args.add(state);
		...
		case MAX:
			...
			break;
		default:
			logger.error("Unknown group function '" + function.value() + "'. Using Equality instead.");
		}
	...
	private void dispatchBindings(String modelName, Item item, List<ModelBinding> bindings) {
		for (ModelBinding binding : bindings) {
			String bindingType = binding.getType();
			String config = binding.getConfig();
			...
			if (reader != null) {
				try {
					...
					logger.error("Binding information of type '" + bindingType + "' for item ‘" + item.getName()
