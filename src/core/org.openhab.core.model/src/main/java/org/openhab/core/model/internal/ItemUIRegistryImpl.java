...
public class ItemUIRegistryImpl implements ItemUIRegistry {
	...
	protected Collection<ItemUIProvider> itemUIProviders;

	protected ItemRegistry itemRegistry;

	protected ObjectFactory sitemapFactory = new ObjectFactory();
	
	public ItemUIRegistryImpl(ItemRegistry itemRegistry, Collection<ItemUIProvider> itemUIProviders) {
		this.itemRegistry = itemRegistry;
		this.itemUIProviders = itemUIProviders;
	}
	...
	public Widget getDefaultWidget(Class<? extends Item> itemType, String itemName) {
		...
		if (itemType.equals(SwitchItem.class)) {
			return sitemapFactory.createSwitch();
		}
		if (itemType.equals(GroupItem.class)) {
			return sitemapFactory.createGroup();
		}
		if (NumberItem.class.isAssignableFrom(itemType)) {
			return sitemapFactory.createText();
		}
		...
		if (itemType.equals(DimmerItem.class)) {
			Slider slider = sitemapFactory.createSlider();
			slider.setSwitchEnabled(true);
			return slider;
		}
		if (itemType.equals(ColorItem.class)) {
			return sitemapFactory.createColorpicker();
		}
	...
	public String getLabel(Widget w) {
		...
		if(itemName!=null && label.contains("[")) {
			...
			try {
				...
				logger.error("Cannot retrieve item for widget {}", w.getClass().getName());
			}
		...
		if(label.contains("[") && label.endsWith("]")) {
			...
			if(matcher.find()) {
				...
				TransformationService transformation = 
					TransformationHelper.getTransformationService(type);
	...
	public String getIcon(Widget w) {
		String widgetTypeName = w.getClass().getSimpleName();
	...
	public State getState(Widget w) {
		...
		if(itemName!=null) {
			try {
				...
				logger.error("Cannot retrieve item '{}' for widget {}", new String[] { itemName, w.getClass().getName() });
			}
		}
	...
	public String getWidgetId(Widget w) {
		String id = "";
		while(w.getParent() instanceof Widget) {
			Widget parent = (Widget) w.getParent();
		...
		if(w.getParent() instanceof Sitemap) {
			Sitemap sitemap = (Sitemap) w.getParent();
