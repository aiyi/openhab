...
public class ItemRegistryImpl implements ItemRegistry, ItemsChangeListener {
	...
	/**
	 * if an EventBus service is available, we provide it to all items, so that
	 * they can communicate over the bus
	 */
	protected EventBus eventBus;

	/** this is our local map in which we store all our items */
	protected Map<ItemProvider, Collection<Item>> itemMap = Collections
			.synchronizedMap(new HashMap<ItemProvider, Collection<Item>>());
	 ...
	 * 
	 * @see
	 * org.openhab.core.internal.items.ItemRegistry#getItem(java.lang.String)
	 */
	@Override
	 ...
	 * 
	 * @see
	 * org.openhab.core.internal.items.ItemRegistry#getItemByPattern(java.lang
	 * .String)
	 */
	@Override
	 ...
	 * 
	 * @see
	 * org.openhab.core.internal.items.ItemRegistry#getItems(java.lang.String)
	 */
	@Override
	...
	private boolean initializeItem(Item item) {
		if (isValidItemName(item.getName())) {
			if (item instanceof GenericItem) {
				...
				genericItem.setEventBus(eventBus);
